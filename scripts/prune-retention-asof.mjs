...
if (!dryRun) {
  ...
  if (existsSync(filingsPath)) {
    try {
      priorFilings = JSON.parse(readFileSync(filingsPath, "utf8")).filings || [];
    } catch {
      priorFilings = [];
    }
  }
  const keptFilings = priorFilings.filter((f) => f?.as_of && f.as_of >= cutoff);
  const filingsDoc = buildFilingsFromAsOfDirs(outDir, withDates, keptFilings);
  writeJson(filingsPath, filingsDoc);
